...
// -------- Registry load/reload (demo only) --------
...
function parseFieldsFromText(text = "") {
  ...
  // Normalize common OCR noise: multiple dots/dashes/underscores and odd spaces
  ...
  // Roll number: multiple label styles
  let roll = null;
  ...
  // Dates: multiple labels and formats
  ...
  // Fallbacks when labels are missing: scan lines heuristically
  const lines = clean.split(/\n+/).map(s => s.trim()).filter(Boolean);

  // If certificate ID missing, look for CERT-XXXX style anywhere
  let certId2 = certId;
  if (!certId2) {
  ...
  // If roll missing, look for token like XX/XXXX/XXXX
  let roll2 = roll;
  if (!roll2) {
...
app.get('/logs', (req, res) => res.json({ items: logs.slice(0, 10) }));
app.get('/registry-count', (req, res) => res.json({ count: registry.length }));
// CSV export of logs (demo-only)
app.get('/export-logs.csv', (req, res) => {
...
app.post('/publish-record', (req, res) => {
  const rec = req.body || {};
